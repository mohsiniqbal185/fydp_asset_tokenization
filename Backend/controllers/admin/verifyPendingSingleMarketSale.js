...
async function verifySingleMarketSale (req, res) {
    ...
    transaction_hash =await transferTokens( Seller_Wallet_Address,Pledger_Wallet_Address,Property_Contract_Address, no_of_tokens);
    console.log('done',transaction_hash);
    const addtransactionquery = 'Insert into token_transaction(property_id,sender_id,receiver_id,no_of_tokens,transaction_hash,payment_id,token_value_id) VALUES(?,?,?,?,?,?,?)';
    ...
    checkfortokenholder = 'SELECT no_of_tokens from token_holders where user_id = ? and property_id=?;';
      ...
      subtracttokensofseller = "update token_holders set no_of_tokens = ? where user_id=? and property_id=?";
  ...
  new Promise((resolve, reject) => {
    // Execute the checkfortokenholder query
    db.query(gettokensofseller, [Pledger_ID, property_id], (err, result) => {
        if (err) {
            reject(err);
        } else {
            // Check if any rows were returned
            if (result[0].no_of_tokens===no_of_tokens) {
                ...
                const existingNoOfTokens = result[0].no_of_tokens;
            const updatedNoOfTokens = existingNoOfTokens - no_of_tokens;
                db.query(subtracttokensofseller, [updatedNoOfTokens,Pledger_ID,property_id], (err, updateResult) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(updateResult);
                ...
                // If no_of_tokens doesn't exist, execute the addnewtokenholder query
                db.query(deletetokensofseller, [Pledger_ID, property_id], (err, addResult) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(addResult);
