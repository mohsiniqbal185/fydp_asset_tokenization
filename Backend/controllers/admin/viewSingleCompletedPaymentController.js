...
const ViewSinglePayment = (req, res) =>{
    ...
    // `;
    const getCompletedPaymentsDataSingleQuery = `SELECT a.payment_id,a.payment_method,CONCAT(b.fname,' ',b.lname) AS Payment_From, a.DateTime, a.payment_amount,a.payment_receipt_file_name,f.payment_status FROM payment a INNER JOIN user b ON a.payment_from=b.user_id INNER JOIN token_buy_request c on a.req_id = c.req_id INNER JOIN payment_status f on c.payment_status = f.payment_status_id WHERE a.payment_id=${paymentID}`;
